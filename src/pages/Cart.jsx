          ...
          <div className='row mb-5'>
            <div className='col-md-8 border rounded p-5'>
                ...
                <tbody>
                  {
                    userCart?.map((item,index)=>(
                      <tr key={item?.id}>
                        <td>{index+1}</td>
                        <td>{item?.title}</td>
                        <td><img width={'50px'} height={'50px'} className='img-fluid' variant="top" src={item?.thumbnail}  alt='Product Image'/></td>
                        <td>
                          <div className='d-flex align-items-center'>
                            <button onClick={()=>handleDecrementQuantity(item)} className='btn fs-1 '>-</button>
                            <input style={{width:'50px'}} className='form-control' type="text" value={item?.quantity} readOnly/>
              ...
              <div className='float-end mt-3'>
                <button onClick={()=>dispatch(emptyCart())} className='btn btn-danger'>Empty Card</button>
